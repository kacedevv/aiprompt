...
import { Sparkles, History, Smile, FileCode, MessageSquareQuote, Terminal } from 'lucide-react';
import { TRANSLATIONS } from './translations';

export default function App() {
  const [activeTab, setActiveTab] = useState<TabView>(TabView.EDITOR);
  const [globalPrompt, setGlobalPrompt] = useState<string>(''); // Shared prompt state
  ...
  // Function to allow Workstation to change tabs (e.g., from Prompt Gen to Editor)
  ...
  const tabs = [
    ...
    { id: TabView.PROFILE, labelKey: 'tab.profile', descKey: 'tab.desc.profile', icon: FileCode },
        ...
        <main className="flex-1 flex overflow-hidden pt-24">
          ...
          <div className="flex-1 flex flex-col overflow-y-auto relative custom-scrollbar">
            ...
            <div className="flex-1">
               <Workstation 
                 key={`${activeTab}-${currentLang}`} // Remount on tab or lang change
                 mode={activeTab} 
                 lang={currentLang}
